...
public class InoutFormat {
    ...
    public static byte[] format(String targetAddress, String dbNumber, int type, int count, int lon, int lat, Date time) {
        ...
        byte[] dbNumberBytes = ConvertUtil.str2Bcd(dbNumber);
        ...
        byte[] messageBytes;
        messageBytes = ByteUtil.byteMerger(MESSAGE_TYPE_INOUT.getBytes(), dbNumberBytes);
        messageBytes = ByteUtil.byteMerger(messageBytes, typeBytes);
